...
필요한 변수 (모두 `.env.example` 참조):

**OWNER/SUPERUSER 계정 — DB 컨테이너 초기화 + 마이그레이션용**
- `DB_GENERAL_PASSWORD`
- `DB_SENSITIVE_PASSWORD`
- `DB_AUDIT_PASSWORD`

**백엔드 트래픽용 제한 계정 (옵션 B — RBAC 권한 분리)**
- `DB_GENERAL_APP_PASSWORD` (general_app 계정)
- `DB_SENSITIVE_APP_PASSWORD` (sensitive_app 계정)
- `DB_AUDIT_WRITER_PASSWORD` (audit_writer 계정)

권한 분리 상세는 [`db/RBAC.md`](RBAC.md) 참조.
...
### 5.2 백엔드용 제한 권한 계정 (옵션 B — RBAC)

3개 DB 모두에 **백엔드 트래픽용 제한 계정**과 **마이그레이션용 OWNER 계정**을 분리합니다.
백엔드(FastAPI)는 평상시 제한 계정으로만 접속하므로,
**SQL Injection이나 코드 실수가 DDL/TRUNCATE 같은 위험 명령으로 이어지지 않습니다.**

| DB | 백엔드 트래픽 계정 | 권한 | 마이그레이션 계정 |
|---|---|---|---|
| general (5432) | `general_app` | SELECT/INSERT/UPDATE/DELETE만 | `heartbeat` (OWNER) |
| sensitive (5433) | `sensitive_app` | SELECT/INSERT/UPDATE/DELETE만 | `heartbeat` (OWNER) |
| audit (5434) | `audit_writer` | **INSERT, SELECT만** (변조 방지) | `heartbeat` (OWNER) |

차단되는 명령:
- DDL: `CREATE/ALTER/DROP TABLE`, `CREATE INDEX` 등
- `TRUNCATE`
- `ALTER TABLE ... DISABLE TRIGGER` (감사 무력화 시도 차단)
- `CREATE OR REPLACE FUNCTION` (트리거 함수 변조 시도 차단)
- `ALTER USER MAPPING` (fdw 자격증명 변조 시도 차단)

F-1/F-2 audit 트리거가 옵션 B 적용 후에도 정상 작동하는 이유는
트리거 함수가 `SECURITY DEFINER`로 선언돼 함수 OWNER(`heartbeat`) 권한으로 실행되기 때문입니다.
자세한 메커니즘과 검증 방법은 [`db/RBAC.md`](RBAC.md) 참조.

최소 권한 원칙(Principle of Least Privilege) + 깊이 있는 방어(Defense in Depth)를 적용한 변조 차단입니다.
...
## 6. 디렉터리 구조
db/
├── audit/init/        # audit DB 초기화 (00~04: audit_writer 포함)
├── general/init/      # general DB 초기화 (01~05: general_app 포함)
├── sensitive/init/    # sensitive DB 초기화 (01~10: sensitive_app 포함)
├── README.md          # 본 문서
├── RBAC.md            # 권한 분리(옵션 B) 전체 설명서
└── AUDIT_TRIGGERS.md  # F-1/F-2 audit 트리거 메커니즘
...
| 2026-06-05 | 옵션 B 도입 — RBAC 권한 분리 (general_app/sensitive_app/audit_writer로 백엔드 트래픽 격리) | Phase 1~4 완료. `db/RBAC.md` 참조 |
